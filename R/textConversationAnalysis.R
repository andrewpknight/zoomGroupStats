...
textConversationAnalysis = function(inputData, inputType, meetingId, speakerId, sentMethod="none") {
  ...
  if(inputType=="transcript") {
    ...
    if(sentMethod %in% c("aws", "syuzhet")) {
      aggSentiment.tr = aggSentiment(inputData, meetingId=meetingId, indivId=NULL, sentMethod)
      aggSentiment.ind = aggSentiment(inputData, meetingId=meetingId, indivId=speakerId, sentMethod)      
    ...
    ########################################				
    if(sentMethod %in% c("aws", "syuzhet")) {
      aggSentiment.ch = aggSentiment(inputData, meetingId = meetingId, indivId = NULL, sentMethod)
      aggSentiment.ind = aggSentiment(inputData, meetingId = meetingId, indivId = speakerId, sentMethod)      
